...
public class PicketLinkIDMServiceImpl implements PicketLinkIDMService, Startable
{
      ...
      InitialContextInitializer dependency) throws Exception
   {
      ...
      // Not ideal, as we are changing field of singleton (could be changed more time with different values for different portal containers)
      ...
      if (config != null)
      {
         ...
         if (apiCacheConfig != null)
         {
            ...
            Cache cache = infinispanCacheFactory.createInfinispanCache(configStream, exoContainerContext.getPortalContainerName(), "api");

            configStream.close();

            // PLIDM API cache
            APICacheProvider apiCacheProvider = infinispanCacheFactory.createAPICacheProvider(staleCacheNodesLinksCleanerDelay, cache);
         ...
         if (storeCacheConfig != null)
         {
            ...
            Cache cache = infinispanCacheFactory.createInfinispanCache(configStream, exoContainerContext.getPortalContainerName(), "store");

            configStream.close();

            IdentityStoreCacheProvider storeCacheProvider = infinispanCacheFactory.createStoreCacheProvider(staleCacheNodesLinksCleanerDelay, cache);
