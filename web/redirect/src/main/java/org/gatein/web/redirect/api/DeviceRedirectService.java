 ...
 * these particular redirects, and provide a means to determine what other alternative sites are available 
...
public interface DeviceRedirectService
{
    ...
    * The redirect is based on the value of the User-Agent for the users browser and/or the any properties
    * determined from the browser. User-Agent string and deviceProperties are optional values and can pass a
    * Null value.
    * 
    * The returned RedirectKey can specify 3 possible situations based on its RedirectType:
    * 1) RedirectType.REDIRECT: The user should be redirected to the SiteKey specified.
    * 2) RedirectType.NOREDIRECT: No redirect should be performed
    * 3) RedirectType.NEEDDEVICEINFO: A redirect cannot currently be determined, but could be determined based on device properties if they were available
    * 
    * @param origin             The name for the original site requested.
    * @param userAgentString    The value User-Agent for the current browser. Null if the User-Agent string is missing from the HTTP headers.
    ...
    * 
    * If no redirection should be performed, this method will return null. For example, the service could be configured
    * to not redirect if there is no corresponding node on the redirect site.
    * 
