...
public class SiteRedirectServiceImpl implements SiteRedirectService, Startable
{
   ...
   {  
      try
      {
         ...
         {
            log.warn("Redirector(" + redirector + ") and DataStorage(" + dataStorage + ") need to be both be set. Site redirection cannot be performed.");
            return null;
         }
      }
      catch (Exception e)
   ...
   public String getRedirectPath(String origin, String redirect, String originRequestPath)
   {
      try
      {
         if (mapper != null)
         {
            ...
            {
               log.debug("Site '" + origin + "' does not have a redirect configured for site '" + redirect + "'. Cannot perform redirect.");
               return null;
            }
         }
         else
