 ...
 */
window.ScrollManager = function(id) {
...
ScrollManager.prototype.init = function() {
	...
	if(!this.arrowsContainer)  {
		...
		var arrowsContainer = gj(this.mainContainer).find(".ScrollButtons");
...
ScrollManager.prototype.loadElements = function(elementClass, clean) {
	...
	this.elements = gj(this.mainContainer).find("." + elementClass);
};
...
ScrollManager.prototype.checkAvailableSpace = function() { // in pixels
	if (!this.maxSpace) {
		this.maxSpace = gj(this.mainContainer).width() - this.getElementSpace(this.arrowsContainer);
	}
...
ScrollManager.prototype.getElementSpace = function(element) {
	...
	if (element) {
		if (this.axis == 0) { // horizontal tabs
			elementSpace += gj(element).outerWidth(true);
			...
			elementSpace += gj(element).outerHeigth(true);
			if (element.decorator) elementSpace += this.getElementSpace(element.decorator);
		}
...
ScrollManager.prototype.checkResize = function() {
	if (this.mainContainer) {
		var tmp = gj("#" + this.mainContainer.id);
		if (!tmp.length) {
		...
		this.arrowsContainer.space = null;
		var curr = gj(this.mainContainer).width() - this.getElementSpace(this.arrowsContainer);
