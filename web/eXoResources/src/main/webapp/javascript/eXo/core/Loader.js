...
eXo.core.Loader = {
  ...
  init : function(scriptName, callback, context, params) {
	  var a = -1;
	  ...
	  if (typeof(scriptName) != "string" && scriptName.length) {
	    ...
	    for (var s = 0; s < _scriptNames.length; s++) {
	      if (this.registered[_scriptNames[s]] || this.durl(_scriptNames[s])) {
	    ...
	    while (typeof(arguments[++a]) == "string") {
	      if (this.registered[scriptName] || this.durl(scriptName)) {
	  ...
	  if (this.durl(scriptName) && scriptName.substring(0, 4) == "url(") {
...
//	      this.callback.apply(context ? context : {}, this.params);
	...
	CallbackItem : function(_callback, _context, _params) {
...
//	    this.callback.apply(ctx, this.params);
	...
	addLoadedScripts : function(scriptPaths, registration) {
